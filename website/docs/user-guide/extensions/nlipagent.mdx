...
[NLIP (Natural Language Interaction Protocol)](https://ecma-international.org/publications-and-standards/standards/ecma-430/){.external-link target="_blank"} is an ECMA-430 standard for interoperable agent communication over HTTP. The `autogen.extensions.nlip` module lets you:
...
from autogen.extensions.nlip import NlipServer
...
```python linenums="1"
from autogen import Agent
from autogen.extensions.nlip import NlipConfig
...
`autogen` has no GroupChat — the geo lookup and the remote weather call are both tools the coordinator's LLM picks between, rather than peer chat participants:
